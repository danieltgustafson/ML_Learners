...
def q_learning(env, init_model, iterations=10000,alpha=.01,gamma=.99925,rar=.9999,radr=.999997):
	...
	for its in range(iterations):
	        ...
	        if len(target_y)>100000 or its==50:
	                ...
	                #ordered=np.argsort(ty)[-(len(ty)/10):,]

	                #pos_t_x,pos_t_y=np.repeat(tx[ordered],5,axis=0),np.repeat(ty[ordered],5,axis=0)
	                #pos=np.where(pos_t_y>0)[0]
	                #pos_t_x,pos_t_y=pos_t_x[pos],pos_t_y[pos]

	                #t_s_x,t_s_y=s_x,s_y=zip(*random.sample(zip(np.vstack((tx,pos_t_x)),np.append(ty,pos_t_y)),min(len(ty),50000)))
	        ...
	        if len(new_y)>100000 or its==50:
	                ...
	                #print len(pos)
	                #s_x,s_y=zip(*random.sample(zip(np.vstack((x,pos_x])),np.append(y,pos_y)),min(len(y),500000)))
	                #s_x,s_y=zip(*random.sample(zip(np.vstack((x,pos_x)),np.append(y,pos_y)),min(len(y),50000)))
